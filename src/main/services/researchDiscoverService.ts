...
export function getDiscoverSections(): DiscoverSection[] {
  let rows: Row[] = []
  try {
    rows = getDb().prepare<[], Row>(`SELECT * FROM research_discover`).all()
  } catch {
    rows = []
  }
  ...
  return DISCOVER_FIELDS.map((f) => {
    ...
    return { fieldId: f.id, label: f.label, papers, fetchedAt: row?.fetchedAt ?? null }
  })
}

// Refreshes stale fields, one at a time through the shared S2 limiter.
// Returns how many were actually refetched.
export async function refreshDiscover(opts: { force?: boolean } = {}): Promise<number> {
  const db = getDb()
  const now = Date.now()
  const existing = new Map(
    getDiscoverSections().map((s) => [s.fieldId, s.fetchedAt])
  )
  const stmt = db.prepare(
    `INSERT INTO research_discover (fieldId, label, papersJson, fetchedAt)
     VALUES (?, ?, ?, ?)
  ...
  for (const field of DISCOVER_FIELDS) {
    ...
    if (!opts.force && fetchedAt !== null && now - fetchedAt < TTL_MS) continue
    const papers = await fetchField(field)
    ...
    stmt.run(field.id, field.label, JSON.stringify(papers), Date.now())
    refreshed++
    // Space the fields out. Ten back-to-back searches is exactly the burst
    // shape that draws 429s.
    await new Promise((r) => setTimeout(r, 1500))
  }
  return refreshed
}

export function discoverIsEmpty(): boolean {
  return getDiscoverSections().every((s) => s.papers.length === 0)
}
